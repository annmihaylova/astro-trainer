...
export function buildSkyChartAnswerMarkers({
    ...
    // markerStatuses передаётся только после общей проверки.
    // Поэтому эталонные ответы появляются именно после "Проверить".
    if (markerStatuses !== undefined) {
        markers.push(
            ...buildCorrectAnswerMarkers({
                session,
                activeTask,
